...
#usage: python ndd_by_nodes_sorting_just_2_Cliffs_average_FIG_sml.py MaxAbs 30 MaxAbs_ndds_ln.txt ChList.txt MaxAbs_ndds_ln_sorting_index_ftol.txt MaxAbs_Cliffs_average_ndds_Control_EtOH.txt
...
plt.imshow(Tippet[sorting_index], cmap=cmap, interpolation='nearest', aspect='auto', vmin=0.0, vmax=3.0)

plt.yticks([])

plt.xticks([])

plt.text(1, 53, 'small', color='#ff7f0e', rotation=90, weight='bold')
plt.text(1, 45, 'medium', color='#2ca02c', rotation=90, weight='bold')
plt.text(1, 35, 'large', color='#d62728', rotation=90, weight='bold')

ax = plt.subplot(143)

ax.set_position([0.8, 0.0925, 0.04, 0.839])

plt.title("$\delta$\n")

Cliffs_values = np.expand_dims(Cliffs_values, axis=1)
...
for i in range(1,-1,-1):
	...
	if (i == 0):
		...
		ax2.set_position([0.185, 0.0925, 0.2, 0.839])
	...
	if (bin_size == 30):
		plt.xticks([0,15],[0.0, np.around(bin_borders[i][1],2)])
